...
import sys
sys.path.append('/root/projects/gpt2_cn/EasyLLM')  # 替换成你的根目录的实际路径
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained("/root/projects/gpt2_cn/LLMs/chatglm2-6b", trust_remote_code=True)  # 创建tokenizer
    model = AutoModel.from_pretrained("/root/projects/gpt2_cn/LLMs/chatglm2-6b", trust_remote_code=True, device='cuda')  # 创建model
    model = model.eval()
    ...
    def continuedQuery():
        ...
        # print("nlg-server-history:", history)
        # print("nlg-server-message:", message)
        response, _ = model.chat(tokenizer, message, history=history)
        # print("nlg-server-response:", response)
        return {"time": api_app.getISOTime(), "content": response}, 200
